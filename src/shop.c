...
void shop_init(shop *s, const float screen_width, const float screen_height) {
    ...
    const float ship_area_width = s->outer_ship_rect.w * 0.35f;
    s->inner_ship_rect = (SDL_FRect){
        .x = s->outer_ship_rect.x + 20,
        .y = s->outer_ship_rect.y + 20,
        .w = ship_area_width - 40,
        .h = s->outer_ship_rect.h - 40
    };

    const float items_area_x = s->inner_ship_rect.x + s->inner_ship_rect.w + 40;
    const float items_area_width = s->outer_ship_rect.x + s->outer_ship_rect.w - items_area_x - 20;
    const float items_area_height = s->outer_ship_rect.h - 40;
    ...
    for (int i = 0; i < s->item_count; i++) {
        const SDL_FRect item_container = {
            .x = items_area_x,
            .y = s->outer_ship_rect.y + 20 + (float)i * (item_height + s->item_spacing),
            .w = items_area_width,
            .h = item_height
        };
...
void shop_render(const shop *s, SDL_Renderer *renderer) {
    ...
    for (int i = 0; i < s->item_count; i++) {
        ...
        render_text_3d(state.renderer, item->title, title_pos, 25.0f, (SDL_Color) {.r = 0, .g = 0, .b = 0, .a = 255});
